...
Before making any UI changes, carefully read and follow the guidelines defined in `DESIGN.md`. Treat it as the single source of truth for all design decisions.

While following `DESIGN.md`, preserve the existing SEAPEDIA brand identity.
...
* Rewrite any copy that contains em dashes using natural sentence structure.

The final UI should feel handcrafted by an experienced product designer, not generated by AI.
...
The interface should look like a high-quality Figma exploration that has been carefully refined over multiple iterations, rather than a generic AI-generated landing page.

Avoid common AI design patterns, including:
...
Prioritize layout quality over visual effects.

Do not add decorative elements unless they improve usability.

Do not introduce new UI patterns that are inconsistent with the existing product.

Before implementing changes:

1. Analyze the current UI.
2. Identify what feels AI-generated or visually weak.
3. Explain what you plan to improve.
4. Then implement the improvements.

Do not modify business logic, routing, APIs, or application behavior.

Only improve presentation, layout, styling, interaction details, accessibility, and UX writing.
